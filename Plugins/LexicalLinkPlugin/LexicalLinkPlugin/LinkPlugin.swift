...
public struct LinkPayload {
    ...
    public init(urlString: String?, label: String?, originalSelection: RangeSelection?) {
...
open class LinkPlugin: Plugin {
  ...
  func insertLink(linkPayload: LinkPayload?, editor: Editor) {
    do {
      ...
      try editor.update {
        getActiveEditorState()?.selection = linkPayload?.originalSelection
          try toggleLink(url: linkPayload?.urlString, label: linkPayload?.label)
    ...
    func toggleLink(url: String?, label: String?) throws {
