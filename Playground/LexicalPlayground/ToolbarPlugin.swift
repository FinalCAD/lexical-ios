...
class ForegroundColorPickerController: UIColorPickerViewController {}
class BackgroundColorPickerController: UIColorPickerViewController {}

public class ToolbarPlugin: NSObject, Plugin {
    ...
    private func setUpToolbar() {
        ...
        let foregroundColor = UIBarButtonItem(
            image: UIImage(systemName: "a.square"),
            style: .plain,
            target: self,
            action: #selector(foregroundColor)
        )
        self.foregroundColorButton = foregroundColor
        
        let backgroundColor = UIBarButtonItem(
            image: UIImage(systemName: "a.square.fill"),
            style: .plain,
            target: self,
            action: #selector(backgroundColor)
        )
        self.backgroundColorButton = backgroundColor
    ...
    private func updateToolbar() {
        if let selection = try? getSelection() as? RangeSelection {
            ...
            self.foregroundColorButton?.tintColor = selection.style.foregroundColor
            self.backgroundColorButton?.tintColor = selection.style.backgroundColor
    ...
    @objc private func foregroundColor() {
        let colorPicker = ForegroundColorPickerController()
        colorPicker.title = "Foreground Color"
        colorPicker.supportsAlpha = false
        colorPicker.delegate = self
        colorPicker.modalPresentationStyle = .popover
        colorPicker.popoverPresentationController?.sourceItem = self.foregroundColorButton
        viewControllerForPresentation?.present(colorPicker, animated: true)
    }
    
    @objc private func backgroundColor() {
        let colorPicker = BackgroundColorPickerController()
        colorPicker.title = "Foreground Color"
        colorPicker.supportsAlpha = false
        colorPicker.delegate = self
        colorPicker.modalPresentationStyle = .popover
        colorPicker.popoverPresentationController?.sourceItem = self.backgroundColorButton
        viewControllerForPresentation?.present(colorPicker, animated: true)
    }
...
extension ToolbarPlugin: UIColorPickerViewControllerDelegate {
    ...
    public func colorPickerViewController(_ viewController: UIColorPickerViewController, didSelect color: UIColor, continuously: Bool) {
        try? editor?.update {
            ...
            case is ForegroundColorPickerController:
                try patchStyleText(
                    styles: [\.foregroundColor: color],
                    editor: editor!
                )
            case is BackgroundColorPickerController:
                try patchStyleText(
